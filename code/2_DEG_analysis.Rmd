...
dir <- paste0("./figures/DESeq2/",params$disease,"/")
ifelse(!dir.exists(dir), dir.create(dir),F) 

#make directory for results
file_dir <- paste0("./result_files/DESeq2/", params$disease,"/")
ifelse(!dir.exists(file_dir), dir.create(file_dir),F) 
...
ggsave(file = paste0(dir,disease_samples,"_PCA.png"), plot = pca, dpi = 300)
...
Volcano 
ggsave(file = paste0(dir, disease_samples,"_Volcano.png"), plot = Volcano, dpi = 300)
```
...
write.csv(ora_deg, file = paste0(file_dir, "/Enrichr_",disease_samples,".csv" ))

### Get top enriched terms per ontology class
 top_gs <- ora_deg %>%
   #get GO Id
 ...
 top_gs
write.csv(ora_deg, file = paste0(file_dir, "/Enrichr_top_",disease_samples,".csv" ))


 tile_plot <- function(ora){
 ...
 top_up <- tile_plot(top_gs %>% filter(Direction == "up")) + labs(subtitle = "Up-regulated DEGs")
 top_up
 top_dn <- tile_plot(top_gs %>% filter(Direction == "down")) + labs(subtitle = "Down-regulated DEGs")
 top_dn

cowplot::plot_grid(top_dn + labs(title = paste(sample_label,"- GO terms for differenially expressed genes")), ncol = 1, align = "v", axis = "lr")

ggsave(file = paste0(dir, disease_samples,"_fdr001_gs_tile_down.png"), dpi = 600, units = "cm", height = 12, width = 42)
...
write.csv(as.data.frame(resOrdered), file = paste0("./result_files/intermediate_files/",disease,"/DESeq2_", disease_samples,".csv"))
