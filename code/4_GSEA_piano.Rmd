...
gscGO <- loadGSC("./metadata/gmt_files/c5.go.bp.v7.4.symbols.gmt", type="gmt") 
 
all_DESeq_res <- read.csv(paste0("./result_files/DESeq2/",disease,"_DESeq_res_all.csv"), header = T, row.names = 1)
...
resListKEGG <- list(gsaResKEGG_mean, gsaResKEGG_median, gsaResKEGG_sum, gsaResKEGG_reporter, gsaResKEGG_tail)

names(resListKEGG) <- c("mean", "median", "sum","reporter","tail")
...
date <- Sys.Date()
save.image(file = paste0(dir, "/R_sessions/GSEA_streptail_score_", dataset, "_", date,".RData"))
```
...
resListGO <- list(gsaResGO_mean, gsaResGO_median, gsaResGO_sum, gsaResGO_reporter, gsaResGO_tail)
names(resListGO) <- c("mean","median","sum", "reporter", "tail")

# resListGO_w_fgsea <- resListGO
# resListGO_w_fgsea$fgsea <- gsaResGO_fgsea
...
write.csv(con_kegg, file = paste0(outdir, dataset, "_reptail_consKEGG_",date,".csv"))
write.csv(con_go, file = paste0(outdir, dataset, "_reptail_consGO_",date,".csv"))
```
