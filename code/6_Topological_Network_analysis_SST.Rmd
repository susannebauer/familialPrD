...
files <- file.path("./salmon/",metadata$nfcore_id, "quant.sf")
...
png(filename = paste0(fig,"/P_corrplot_",celltype,".png"), height = 900, width = 900)
...
plot_dist_fdr <- ggplot(FDR, aes(x = R_fdr, fill = col))+
  ...
  labs(title = "Distribution correlation coefficients after multiple testing correction",
       ...
       fill = "Threshold")
plot_dist_fdr
ggsave(filename = paste0(fig,"/Distr_R_", celltype,"_",date, ".png"), plot = plot_dist_fdr, dpi = 320)
...
save.image(file = paste0("./R_sessions/Network_analysis_", celltype,"_", date, ".RData"))
```
...
stat_table
save_kable(stat_table, file = paste0(fig,"/Network_stats_",celltype,"_",date,".png"))
...
ggsave(filename = paste0(fig,"/Degree_distr_", celltype,"_",date, ".png"), plot = deg_dist, dpi = 320, units = "cm", width = 25, height = 15)
```
...
ggsave(filename = paste0(fig,"/prob_plot_", celltype,"_",date, ".png"), plot = prob_plot, dpi = 320)
...
ggsave(filename = paste0(fig,"/raw_centralities_", celltype,"_",date, ".png"), plot = plot_raw_centrality, dpi = 320, units = "cm", height = 20, width = 15)
...
ggsave(filename = paste0(fig,"/Module_size_", celltype,"_",date, ".png"), plot = module_size, dpi = 320)
...
print_enrichr_plots <- function(input, name){
  ...
  ggsave(filename = paste0(fig,"/EnrichedTerms_",name,"_",celltype,"_",date,".png"), plot = plot, dpi = 320, units = "cm", width = 60, height = 30)
  return(plot)
}
...
CJD <- read.csv(paste0("./result_files/DESeq/CJD_DEGs.csv"), stringsAsFactors = T, sep = ",")
FFI <- read.csv(paste0("./result_files/DESeq/FFI_DEGs.csv"), stringsAsFactors = T, sep = ",")
...
FFI_stats <- read.csv(paste0("./result_files/DESeq/FFI_DESeq_res_all.csv"))
FFI_stats <- FFI_stats %>% 
...
CJD_stats <- read.csv(paste0("./result_files/DESeq/CJD_DESeq_res_all.csv"))
CJD_stats <- CJD_stats %>% 
  filter(Celltype == celltype) %>%
  filter(symbol %in% pos_w_nodes_annot$name) %>%
...
writeLines(capture.output(run_info), paste0("./documentation/Run_info_Network_analysis_",celltype, "_", date,".txt"))
